    ...
    public static class Initiator extends FlowLogic<ExampleFlowResult> {
        ...
        @Override public ExampleFlowResult call() {
            ...
            try {
                ...
                // Add a timestamp (the contract code in PurchaseOrderContract mandates that ExampleStates are timestamped).
    ...
    public static class Acceptor extends FlowLogic<ExampleFlowResult> {
        ...
        @Override public ExampleFlowResult call() {
            try {
                ...
                final DigitalSignature.WithKey mySig = CryptoUtilities.signWithECDSA(keyPair, partSignedTx.getId().getBytes());
                // Add our signature to the transaction.
                final SignedTransaction signedTx = partSignedTx.plus(mySig);

                // Stage 7.
                progressTracker.setCurrentStep(FINALISING_TRANSACTION);
                final Set<Party> participants = Stream.of(getServiceHub().getMyInfo().getLegalIdentity(), otherParty).collect(Collectors.toSet());
